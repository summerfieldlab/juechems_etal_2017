...
function [params,fit] = cval_pt_aspiration(data,use,sub,flag,init_params,bnd_low,bnd_up,crossval,n_Iter)
...
% LOG LIKELIHOOD FUNCTION
    ...
    function gamble_utility = get_utility(parms)
        ...
        % Basically this is an expected payoff per trial
        asp = parms(4) * data.vb_distart(use);
       
        gamble_utility = get_pt_prob(prob,gamma) .* get_pt_utility(gain + cumu - asp,[alpha,lambda]) ...
                       + get_pt_prob((1-prob),gamma) .* get_pt_utility(loss + cumu - asp,[alpha,lambda]);
        gamble_utility = gamble_utility - get_pt_utility(-0.1 + cumu - asp,[alpha,lambda]);
