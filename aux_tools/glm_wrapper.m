...
for sub = subjects
    ...
    % Do regression
    [B(sub,:), DEV(sub), STAT(sub)] = glmfit(pred_z',use_dep',glm_options{1},glm_options{2:end});%'binomial','link','logit');
end
...
function vals = fetch_values(names)
    if length(names) == 1
        ...
        vals = vertcat(fetch_values(names(1:end-1)),data.(names{end}));
    end
end
end
